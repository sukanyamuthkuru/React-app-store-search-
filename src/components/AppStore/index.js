...
class AppStore extends Component {
  ...
  getFilteredApps = activeTabId => {
    const fItems = appsList.filter(each => each.category === activeTabId)
  ...
  render() {
    ...
    const filteredTabItems = this.getFilteredApps(activeTabId)
    const lastFilter = filteredTabItems.filter(each =>
    ...
    return (
      <div className="app-container">
        ...
        <div className="input-styling-container">
          <input
            type="search"
        ...
        <ul className="app-item-container">
          {lastFilter.map(eachItem => (
            <AppItem key={eachItem.appId} detailsOfApp={eachItem} />
          ))}
        </ul>
      </div>
    )
